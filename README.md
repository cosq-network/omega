...
- **AArch64/RISC-V Display Integration**: Shared FDT walker and boot-pointer handoff, Device Tree `simple-framebuffer` HALs, pixel-format metadata, portable framebuffer console routing, and safe serial fallback. VirtIO-GPU remains planned.
...
| [`docs/DISPLAY_AARCH64_RISCV_PLAN.md`](docs/DISPLAY_AARCH64_RISCV_PLAN.md) | AArch64 & RISC-V 64 | 7.2b | **SimpleFb HAL integrated; VirtIO-GPU planned** |
...
| **Architecture HALs** | `kernel/arch/{aarch64,riscv64}/display.cpp` | SimpleFb probe, identity-map bring-up, serial fallback; VirtIO-GPU pending |
